...
confs = __import__('config.config', fromlist=['conf'])
conf = confs.conf
print(conf)
...
dataset, num_node_features, num_edge_features, num_graph_features = get_dataset(args.pro_dataset_path, conf['dataset'], conf['graph_level_feature'])
print("======================================")
print("=====Total number of graphs in", conf['dataset'],":", len(dataset), "=====")
assert conf['num_tasks'] == dataset[0].y.shape[-1]
train_dataset, val_dataset, test_dataset = split_data(args.ori_dataset_path, conf['dataset'], dataset, conf['split'], conf['seed'], split_size=conf['split_ratio'])
print("=====Total number of training graphs in", conf['dataset'] ,":", len(train_dataset), "=====")
print("=====Total number of validation graphs in", conf['dataset'] ,":", len(val_dataset), "=====")
print("=====Total number of test graphs in", conf['dataset'] ,":", len(test_dataset), "=====")
print("======================================")
